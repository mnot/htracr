...
//    self.h = Math.max(y + self.margin[2], self.h);
  ...
  inspect_element: function (e) {
    ...
    self.colour_adjust(e, 'stroke', 70);
    self.msg.html(msg);
  ...
  handle_key: function (ch) {
    ...
    switch (ch.keyCode) {
      ...
      case 39: // right
        ...
        break;
      case 38: // up
        switch (self.cursor_loc['kind']) {
          case 'packets':
//            self.cursor_loc['kind'] = 'http-reqs';
            break;
          case 'http_reqs':
            return true;
          case 'http_ress':
            self.cursor_loc['x'] = self.loc_to_element(self.cursor_loc).pkt;
            self.cursor_loc['kind'] = 'packets';
            break;
          default:
            console.log('Unknown cursor kind: ' + self.cursor_loc['kind']);
            return true;
        }
        break;
      case 40: // down
