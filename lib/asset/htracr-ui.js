...
htracr.ui = function () {
  ...
  // object for user interaction
    ...
    show_referers: true,                // default
    show_locations: true,               // default
    ...
    // change the zoom level and re-render
      ...
      jQuery(eid).toggle(function (ev) {
        htracr.ui[arr_name].forEach(function (e) {
          e.hide();
        });
  ...
  var di = {
    ...
    save: function (name, value, cursor, single) {
      if (value) {
        ...
        if (single === true) {
          htracr.ui.urls[name][value] = cursor;
        } else {
          if (! htracr.ui.urls[name][value]) {
            htracr.ui.urls[name][value] = [];
          }
          htracr.ui.urls[name][value].push(cursor);
        }
      }
    }
  };
  ...
  function time_x (t) {
    ...
    var delta = t - htracr.ui.capture_idx.start;
    if (delta < 0) {
