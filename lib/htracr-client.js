...
//		self.h = Math.max(y + margin[2], self.h);
		...
		for (ref in self.refs) {
			if (self.urls[ref]) {
				self.refs[ref].forEach(function(r) {
				  ...
				  console.log("drawing referer from " + s + " to " + e);
  ...
  _h_timeout: undefined,
	...
	draw_bookends: function() {
		...
		self.paper.line([start_x, margin[0]], [start_x, end_y]).attr(a);
		for (var i = self.first; i < self.last; i += 1000) {
		  ...
		  self.paper.text(i_x, end_y + 10,
		    ((i - self.first) / 1000) + "s").attr({
		    fill: "white",
		    "font-size": "16",
		    "opacity": ".5",
		  });
		}
		self.paper.line([end_x, margin[0]], [end_x, end_y]).attr(a);
	},
