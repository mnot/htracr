...
//		self.h = Math.max(y + margin[2], self.h);
  ...
  draw_conn: function(conn_start, conn_end, y) {
		var self = this;
		var start_x = self.time_x(conn_start);
  ...
  _h_timeout: undefined,
	...
	draw_bookends: function() {
		...
		for (var i = self.first; i < self.last; i += 1000) {
		  var i_x = self.time_x(i);
		  self.paper.line([i_x, margin[0]], [i_x, self.h]).attr({
		    stroke: "#444",
		    "stroke-width": "1",
		  });
		}
