...
var htracr = {
    ...
    for (loc in self.locs) {
      if (self.urls[loc]) {
        self.paper.curve(self.urls[loc], self.locs[loc]).attr({
						"stroke": "#9c9",
						"stroke-width": "3",
						"opacity": "0.7",
        });
  ...
  // we can render().
	process_conns: function (server_conns) {
		var self = this;
		var y = margin[0];
		self.first = self.start_time(server_conns);
		self.last = self.end_time(server_conns);
		...
		self.w = ((self.last - self.first) / 1000 * pix_per_sec) + margin[1] + margin[3];
  		...
  		function show_hdrs(hdrs) {
			...
			if (msg_type == 'req') {
			  ...
			  if (details.method != 'GET') {
			    self.paper.text(start+20, msg_y-10, details.method).attr(a);
			  }
			}	else {
			  desc = "HTTP/" + details.http_version + " " 
			       + details.status_code 
			  if (details.status_code != 200) {
			    self.paper.text(start+16, msg_y+10, details.status_code).attr(a);
			  }
			}
  ...
  _h_timeout: undefined,
	...
	draw_bookends: function() {
		...
		self.paper.line([start_x, margin[0]], [start_x, self.h]).attr(a);
		self.paper.line([end_x, margin[0]], [end_x, self.h]).attr(a);
	},
	...
	start_time: function (server_conns) {
		var self = this;
		var f = undefined;
		for (server in server_conns) {
			for (connection in server_conns[server]) {
				var t = server_conns[server][connection][0].time;
				if (! f || (t < f)) {
					f = t;
				}
			}
  ...
  unpulse_logo: function (done) {
	...
	end_time: function (server_conns) {
		var self = this;
		var l = undefined;
		for (server in server_conns) {
			for (connection in server_conns[server]) {
				var t = server_conns[server][connection].slice(-1)[0].time;
				if (! l || (t > l)) {
					l = t;
				}
			}
