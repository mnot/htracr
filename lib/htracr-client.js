...
var htracr = {
  ...
  _h_timeout: undefined,
	...
	draw_bookends: function() {
		...
		var s = self.paper.line([start_x, margin[0]], [start_x, self.h]).attr(a);
		s.blur(4);
		var e = self.paper.line([end_x, margin[0]], [end_x, self.h]).attr(a);
		e.blur(4);
	},
...
function update_state() {
	...
	req.onreadystatechange = function () {
		if (req.readyState == 4) {
			...
			htracr.process_conns(eval("(" + req.responseText + ")"));
			htracr.render();
			console.log('updated.');
		}; 
	};
