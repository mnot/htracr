...
jQuery(document).ready(function() {
	...
	toggle_array("#refs", "ref_elements");
	toggle_array("#redirs", "loc_elements");
...
function toggle_array(eid, arr_name) {
...
//		self.h = Math.max(y + margin[2], self.h);
    ...
    self.conns.forEach(function(c) {
      ...
      c.packets.forEach(function(p) {
        self.draw_packet(p, c.y);
  		...
  		c.http_reqs.forEach(function(req) {
  			self.draw_http_message('req', c.y, req);
  		});

  		c.http_ress.forEach(function(res) {
  			self.draw_http_message('res', c.y, res);
  		});
    });
  ...
  draw_conn: function(conn) {
		...
		self.hover(conn_e, "RTT: " + conn.rtt);
  },
  ...
  rewind_packets: function(packets, msg) {
	...
	draw_http_message: function(msg_type, y, r) {
	    var self = this;
			var a = {
			  'fill': 'white',
			  'opacity': '0.4', 
			...
			var desc;
			var start_x = self.time_x(r.start) || margin[3]
			var end_x = self.time_x(r.end) || self.w - margin[1]
  		...
  		function show_hdrs(hdrs) {
			...
			if (msg_type == 'req') {
				desc = r.msg.method + " " 
				     + r.msg.url 
		         + " HTTP/" + r.msg.http_version 
		         + "<br>" + show_hdrs(r.msg.headers);
			  if (r.msg.method != 'GET') {
			    self.paper.text(start_x+20, msg_y-10, r.msg.method).attr(a);
			  }
			}	else {
			  desc = "HTTP/" + r.msg.http_version + " " 
			       + r.msg.status_code 
			  if (r.msg.status_code != 200) {
			    self.paper.text(
			      start_x+16, msg_y+10, r.msg.status_code
			    ).attr(a);
			  }
			}
 		  desc += "<br>" + show_hdrs(r.msg.headers);
  ...
  // of servers, based upon first connection time.
	...
	process_conn: function (conn, y) {
		...
		conn.forEach(function(item) {
		  ...
		  if (! self.last || item.time > self.last) {
			...
			switch (item.what) {
				...
				case "http-req-start":
					http_reqs.push({
					  start: self.rewind_packets(packets, item),
					  end: null,
					  msg: item,
					});
					break;
		...
		http_reqs.forEach(function(req) {
			var url = "http://" + req.msg.headers.Host + req.msg.url;
			self.urls[url] = [req.end, self.http_msg_y(y, 'req')];

			var ref = req.msg.headers.Referer;
			if (ref) {
    ...
    http_ress.forEach(function(res) {
      var loc = res.msg.headers.Location;
      if (loc) {
